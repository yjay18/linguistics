...
Milestones 2–6 are committed and pushed through `61e60ad`; Milestone 7 review/progress is `d182749`, local-data/release hardening is `34d84fd`, and the final-design slice is `f8cde75`. The design slice adds a centralized light/dark design system, polished shell and feature surfaces, short purposeful page/control/conversation motion, a saved reduced-motion preference with pre-profile override, backward-compatible schema-5 persistence, and automated semantic contrast/resource checks. Locked restore, zero-warning Release build, 171 automated tests, formatter, framework-dependent publish, notice hashes, and artifact-scope inspection pass locally. GitHub Actions run `32342596021` passes locked restore, Release build, tests, and publish on both macOS and Windows for exact commit `f8cde752309e50fe3bc3d1a7a4c490562e497c29`. The normal app still rejects the bundled machine-validated packs for learner use; only a synthetic test-approved copy may exercise learner-facing content. The Mac remains locked, its native renderer cannot start in that state, and no usable German Whisper model or Windows desktop is configured.
